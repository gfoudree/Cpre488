...
int main() {
    ...
    // VDMA_MM2S_CR[0]             = CHANGE_ME; // Read Channel Circular/Park Mode and enable the channel
    // VDMA_MM2S_REG_INDEX[0]      = CHANGE_ME; // Read Channel Reg_Index
    // VDMA_MM2S_START_ADDRESS1[0] = CHANGE_ME; // VDMA Read channel Frame buffer Start Add 1
    // VDMA_MM2S_FRMDLY_STRIDE[0]  = CHANGE_ME; // // 640*2 bytes, VDMA  Read channel FRM_Delay, and Stride
    // VDMA_MM2S_HSIZE[0]          = CHANGE_ME; // 640*2 bytes, Read channel HSIZE
    // VDMA_MM2S_VSIZE[0]          = CHANGE_ME; // 480 lines, Read channel VSIZE  (Note: Starts VDMA transaction)
