...
void resizePixels(u16 *pixels, u16 *resized, int w1,int h1,int w2,int h2) {
    u16 temp[w2*h2];
    ...
    memcpy(resized, temp, w2*h2*sizeof(u16));
}
...
void NESCore_Callback_OutputFrame(word *WorkFrame) {
	...
	for (i = 0 ; i < NES_DISP_HEIGHT - 1; i++) {
		for (j = 0; j < NES_DISP_WIDTH - 1; j++) {
			...
			if (j < 2) {
				...
				int newI = i*2;
				int newJ = j*2+68;
				VGA_frame[newI][newJ] = tpixel;
				VGA_frame[newI+1][newJ] = tpixel;
				VGA_frame[newI][newJ+1] = tpixel;
				VGA_frame[newI+1][newJ+1] = tpixel;
			}
		}
	}
